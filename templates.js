...
function renderSocialRow(data, options, align = "left") {
  ...
  if (style === "text" || (outlookSafe && (style === "badges" || style === "icons"))) {
    ...
    return `<div style="margin-top:8px;font-size:12.5px;line-height:1.25;${alignCss}">${html}</div>`;
  }
  ...
  const cells = links
    .map((l) => {
      ...
      const icon = SOCIAL_ICON_SVG[l.key] || "";
  ...
  const tableMargin = align === "center" ? "margin:10px auto 0;" : "margin-top:10px;";
  return `
    <table cellpadding="0" cellspacing="0" border="0" style="${tableMargin}">
      <tr>${cells}</tr>
    </table>`;
}
...
function templateInitials(data, options) {
  const base = richNameBlock(data, options);
  const lines = buildLines(data);
  const social = renderSocialRow(data, options);
  const accent = options.accentColor;
  ...
  const nameSize = compact ? 14 : 16;
  const small = compact ? 12 : 13;
  const badge = compact ? 54 : 64;
  const initials = initialsFromFullName(data?.fullName);
  const taglineFooter = renderTaglineFooter(base, options);
  const brandingFooter = renderBrandingFooter(options);
  ...
  const contactHtml = lines
    .map((l) => {
      ...
      const content = l.href
        ? `<a href="${esc(l.href)}" style="color:${esc(base.textColor)};text-decoration:none;">${val}</a>`
        : val;
      return `<tr><td style="padding:0 0 4px 0;vertical-align:top;"><span style="color:rgba(0,0,0,0.55);">${esc(
        l.label
      )}</span> ${content}</td></tr>`;
    })
    .join("");
        ...
        <tr>
          <td style="padding:${pad}px;background:#ffffff;">
            <table cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td style="padding:0;vertical-align:top;">
                  <div style="width:${badge}px;height:${badge}px;background:${esc(
    accent
  )};border-radius:${outlookSafe ? 0 : 16}px;text-align:center;line-height:${badge}px;">
                    <span style="font-size:${compact ? 18 : 20}px;font-weight:900;color:#ffffff;letter-spacing:0.06em;">${esc(
    initials
  )}</span>
                  </div>
                </td>
                ...
                <td style="padding:0;vertical-align:top;">
                  <div style="font-size:${nameSize}px;font-weight:900;line-height:1.2;margin:0;">${base.nameLine}</div>
                  ${
                    base.jobLine || base.companyLine
                      ? `<div style="margin-top:6px;font-size:${small}px;line-height:1.3;color:rgba(0,0,0,0.70);">
                          ${[base.jobLine, base.companyLine].filter(Boolean).join(" · ")}
...
function templateInline(data, options) {
  const base = richNameBlock(data, options);
  const lines = buildLines(data);
  const social = renderSocialRow(data, options);
  const compact = options.density === "compact";
  ...
  const inline = lines
    .map((l) => {
      const val = esc(l.text);
      const content = l.href ? `<a href="${esc(l.href)}" style="color:${esc(base.textColor)};text-decoration:none;">${val}</a>` : val;
      return `<span style="white-space:nowrap;"><span style="color:rgba(0,0,0,0.55);">${esc(l.label)}</span> ${content}</span>`;
    })
    .join(`<span style="color:rgba(0,0,0,0.28);"> · </span>`);
...
<table cellpadding="0" cellspacing="0" border="0" style="font-family:${esc(base.font)};color:${esc(base.textColor)};">
  <tr>
    <td style="padding:0;">
      ...
      ${
        base.jobLine || base.companyLine
          ? `<div style="margin-top:6px;font-size:${small}px;line-height:1.3;color:rgba(0,0,0,0.72);">
              ${[base.jobLine, base.companyLine].filter(Boolean).join(" · ")}
            </div>`
          : ""
      }
      ${inline ? `<div style="margin-top:10px;font-size:${small}px;line-height:1.35;">${inline}</div>` : ""}
      ${social}
      ${taglineFooter}
      ${brandingFooter}
    </td>
...
function templateDarkHeader(data, options) {
  const base = richNameBlock(data, options);
  const lines = buildLines(data);
  const social = renderSocialRow(data, options);
  const accent = options.accentColor;
...
<table cellpadding="0" cellspacing="0" border="0" style="font-family:${esc(base.font)};color:${esc(base.textColor)};">
  <tr>
    <td style="padding:0;">
      <table cellpadding="0" cellspacing="0" border="0" style="border:1px solid rgba(0,0,0,0.12);border-radius:${radius}px;overflow:hidden;">
        <tr>
          <td style="padding:${pad}px;background:${headerBg};color:#ffffff;vertical-align:top;">
            <div style="font-size:${nameSize}px;font-weight:900;line-height:1.2;margin:0;">${base.nameLine}</div>
            ${
              base.jobLine || base.companyLine
                ? `<div style="margin-top:6px;font-size:${small}px;line-height:1.3;opacity:0.9;">
                    ${[base.jobLine, base.companyLine].filter(Boolean).join(" · ")}
                  </div>`
                : ""
            }
            <div style="margin-top:${compact ? 10 : 12}px;height:3px;width:${compact ? 64 : 78}px;background:${esc(accent)};border-radius:999px;"></div>
          </td>
          <td style="padding:${pad}px;background:#ffffff;vertical-align:top;">
            ${
              contactHtml
    ...
    .map((l) => {
      ...
      return `<tr>
        <td style="padding:0 0 6px 0;vertical-align:top;">
          <span style="display:inline-block;background:rgba(0,0,0,0.06);border:1px solid rgba(0,0,0,0.08);border-radius:999px;padding:3px 8px;font-size:${compact ? 10.5 : 11}px;line-height:1;color:rgba(0,0,0,0.70);font-weight:700;">
            ${esc(l.label)}
          </span>
        </td>
        <td style="padding:0 0 6px 10px;vertical-align:top;font-size:${small}px;line-height:1.25;">${content}</td>
      </tr>`;
    })
    .join("");
...
<table cellpadding="0" cellspacing="0" border="0" style="font-family:${esc(base.font)};color:${esc(base.textColor)};">
  <tr>
    <td style="padding:0;">
      <table cellpadding="0" cellspacing="0" border="0" style="border:2px solid ${esc(accent)};border-radius:${radius}px;overflow:hidden;">
        <tr>
          <td style="padding:${pad}px;background:${esc(accent)};color:#ffffff;">
            <div style="font-size:${nameSize}px;font-weight:950;line-height:1.15;margin:0;letter-spacing:0.01em;">${base.nameLine}</div>
            ${
              base.jobLine || base.companyLine
                ? `<div style="margin-top:6px;font-size:${small}px;line-height:1.3;opacity:0.95;">
                    ${[base.jobLine, base.companyLine].filter(Boolean).join(" · ")}
...
function templatePoster(data, options) {
  ...
  const social = renderSocialRow(data, options, "center");
  const nameSize = compact ? 16 : 18;
  const small = compact ? 12 : 13;
  const pad = compact ? 12 : 14;
  const radius = outlookSafe ? 0 : 18;
  ...
  const contactItems = lines
    .map((l) => {
      ...
      return `<div style="margin-top:6px;">
        <span style="color:rgba(0,0,0,0.55);">${esc(l.label)}</span> ${content}
      </div>`;
    })
    .join("");
...
<table cellpadding="0" cellspacing="0" border="0" style="font-family:${esc(base.font)};color:${esc(base.textColor)};">
  <tr>
    <td style="padding:0;">
      <table cellpadding="0" cellspacing="0" border="0" style="border:1px dashed rgba(0,0,0,0.28);border-radius:${radius}px;overflow:hidden;background:#ffffff;">
        <tr>
          <td style="padding:${pad}px;text-align:center;">
            <div style="font-size:${nameSize}px;font-weight:950;line-height:1.1;margin:0;letter-spacing:0.06em;text-transform:uppercase;">${base.nameLine}</div>
            ${
              base.jobLine || base.companyLine
                ? `<div style="margin-top:8px;font-size:${small}px;line-height:1.35;color:rgba(0,0,0,0.72);">
                    ${[base.jobLine, base.companyLine].filter(Boolean).join(" · ")}
                  </div>`
                : ""
            }
            <div style="margin:12px auto 0;height:3px;width:${compact ? 92 : 120}px;background:${esc(accent)};border-radius:999px;"></div>
            ${contactItems ? `<div style="margin-top:12px;font-size:${small}px;line-height:1.35;">${contactItems}</div>` : ""}
            ${social}
            ${taglineFooter ? `<div style="text-align:center;">${taglineFooter}</div>` : ""}
            ${brandingFooter ? `<div style="text-align:center;">${brandingFooter}</div>` : ""}
          </td>
        </tr>
      </table>
    </td>
...
export const TEMPLATES = [
  ...
  { id: "initials", name: "Initials", description: "Mutig mit Initialen-Badge.", render: templateInitials },
  { id: "inline", name: "Inline", description: "Sehr kompakt in einer Zeile.", render: templateInline },
  { id: "darkheader", name: "Dark Header", description: "Kräftiger Kopf, klarer Inhalt.", render: templateDarkHeader },
  {
    id: "bubble",
    name: "Ribbon",
